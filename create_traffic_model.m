...
function create_api_data_interface(modelName)
    ...
    % MATLAB Function bloğu ile API entegrasyonu ekleme
    try
        ...
        % Overpass API ile iletişim kuracak MATLAB Function kodu
        fcnCode = ['function [north, south, east, west] = fcn()\n',...
                   '    persistent data last_time;\n',...
                   '    persistent api_url update_rate;\n',...
                   '    \n',...
                   '    % İlk çalıştırma ayarları\n',...
                   '    if isempty(data)\n',...
                   '        data = struct(''north'', 0.5, ''south'', 0.6, ''east'', 0.4, ''west'', 0.3);\n',...
                   '        last_time = tic;\n',...
                   '        api_url = ''https://overpass-api.de/api/interpreter'';\n',...
                   '        update_rate = 10; % saniyede bir güncelle\n',...
                   '    end\n',...
                   '    \n',...
                   '    % API''den veri çekme (10 saniyede bir)\n',...
                   '    current_time = toc(last_time);\n',...
                   '    if current_time >= update_rate\n',...
                   '        try\n',...
                   '            % API veri çekme girişimi\n',...
                   '            disp(''API''den veri çekiliyor...'');\n',...
                   '            % Koordinat verileri - İstanbul Taksim için örnek\n',...
                   '            lat = 41.0370;\n',...
                   '            lon = 28.9850;\n',...
                   '            radius = 500; % metre\n',...
                   '            \n',...
                   '            % Overpass API sorgusu\n',...
                   '            query = sprintf(''[out:json];way(around:%d,%f,%f)[highway];out;'', radius, lat, lon);\n',...
                   '            options = weboptions(''ContentType'', ''json'');\n',...
                   '            \n',...
                   '            % Web isteği gönderme\n',...
                   '            try\n',...
                   '                response = webwrite(api_url, ''data'', query, options);\n',...
                   '                disp(''API verisi başarıyla alındı'');\n',...
                   '                \n',...
                   '                % Veri analizi ve işleme\n',...
                   '                if isfield(response, ''elements'') && ~isempty(response.elements)\n',...
                   '                    highways = length(response.elements);\n',...
                   '                    % Basit bir trafik yoğunluğu hesaplama - örnek\n',...
                   '                    density = min(1, highways / 50);\n',...
                   '                    \n',...
                   '                    % Farklı yönler için yoğunlukları güncelle (gerçekte coğrafi hesaplamalar gerekir)\n',...
                   '                    data.north = density * 0.9 + rand * 0.1;\n',...
                   '                    data.south = density * 0.8 + rand * 0.2;\n',...
                   '                    data.east = density * 1.1 + rand * 0.1;\n',...
                   '                    data.west = density * 0.7 + rand * 0.3;\n',...
                   '                end\n',...
                   '            catch web_err\n',...
                   '                disp([''API web isteği hatası: '', web_err.message]);\n',...
                   '                % Hata durumunda hafif rastgele değişiklikler\n',...
                   '                data.north = data.north + (rand - 0.5) * 0.1;\n',...
                   '                data.south = data.south + (rand - 0.5) * 0.1;\n',...
                   '                data.east = data.east + (rand - 0.5) * 0.1;\n',...
                   '                data.west = data.west + (rand - 0.5) * 0.1;\n',...
                   '            end\n',...
                   '        catch api_err\n',...
                   '            disp([''API veri işleme hatası: '', api_err.message]);\n',...
                   '        end\n',...
                   '        \n',...
                   '        % Değerlerin 0-1 aralığında kalmasını sağla\n',...
                   '        data.north = max(0, min(1, data.north));\n',...
                   '        data.south = max(0, min(1, data.south));\n',...
                   '        data.east = max(0, min(1, data.east));\n',...
                   '        data.west = max(0, min(1, data.west));\n',...
                   '        \n',...
        ...
        add_line(subsystemPath, 'Get_API_Data/1', 'Mix_North/1', 'autorouting', 'on');
        add_line(subsystemPath, 'Get_API_Data/2', 'Mix_South/1', 'autorouting', 'on');
        add_line(subsystemPath, 'Get_API_Data/3', 'Mix_East/1', 'autorouting', 'on');
        add_line(subsystemPath, 'Get_API_Data/4', 'Mix_West/1', 'autorouting', 'on');
        ...
        fprintf('API entegrasyonu başarıyla etkinleştirildi.\n');
    catch e
